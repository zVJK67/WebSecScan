...
def _run_openssl(host: str, port: int = 443, proto_flag: Optional[str] = None, timeout: int = OPENSSSL_TIMEOUT) -> Optional[str]:
    ...
    # '-brief' is optional and some builds don't support it; keep command minimal
    if proto_flag:
...
def _probe_cipher_and_tempkey(host: str, port: int = 443) -> Dict[str, Any]:
    ...
    Returns a dict with keys:
      negotiated_cipher, weak_ciphers (list), forward_secrecy (bool), dh_key_bits (int or None), openssl_available, openssl_output
    """
    res = {
...
def run_ssl_check(target_url: str, verbose: bool = False) -> List[Dict[str, Any]]:
    ...
    cipher_probe = _probe_cipher_and_tempkey(host, port)
